...
export function TopicsLayout({ sidebar, children, tracePanel, contentPadding = true }: TopicsLayoutProps) {
  ...
  return (
    <div className="bg-background text-muted-foreground flex h-full min-h-0">
      {sidebar ? <aside className="border-border min-h-0 w-88 shrink-0 border-r">{sidebar}</aside> : null}
      {hasContent ? (

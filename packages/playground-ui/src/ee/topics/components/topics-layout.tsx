...
export function TopicsLayout({ sidebar, children, tracePanel, contentPadding = true }: TopicsLayoutProps) {
  ...
  return (
    <div className="flex h-full min-h-0 bg-background text-muted-foreground">
      {sidebar ? <aside className="min-h-0 w-88 shrink-0 border-r border-border">{sidebar}</aside> : null}
      {hasContent ? (
        <div className={contentPadding ? 'min-w-0 flex-1 p-4' : 'min-w-0 flex-1'}>

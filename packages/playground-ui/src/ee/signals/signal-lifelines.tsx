...
export function SignalLifelines({
  ...
  return (
    <section aria-label={`${label} lifelines`} className="min-w-0">
      <h3 className="text-column font-mono tracking-widest uppercase" style={{ color: nodeColor(hue) }}>
        <Tooltip>
          <TooltipTrigger
            render={
              <button
      ...
      {isCollapsed ? undefined : rows.length === 0 ? (
        <p className="text-muted-foreground text-caption mt-2">No themes in these landmarks.</p>
      ) : (

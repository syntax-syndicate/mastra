...
describe('schemas', () => {
  ...
  describe('name validation', () => {
    ...
    describe('valid names', () => {
      it('should accept simple lowercase name', () => {
        const result = validateSkillMetadata({ metadata: { name: 'myskill', description: validDescription } });
        expect(result.valid).toBe(true);
        expect(result.errors).toHaveLength(0);
      });

      it('should accept name with hyphens', () => {
        const result = validateSkillMetadata({ metadata: { name: 'my-skill-name', description: validDescription } });
        expect(result.valid).toBe(true);
      });

      it('should accept name with numbers', () => {
        const result1 = validateSkillMetadata({ metadata: { name: 'skill123', description: validDescription } });
        const result2 = validateSkillMetadata({ metadata: { name: '123skill', description: validDescription } });
        expect(result1.valid).toBe(true);
        expect(result2.valid).toBe(true);
      });

      it('should accept name with hyphens and numbers', () => {
        const result = validateSkillMetadata({ metadata: { name: 'my-skill-v2', description: validDescription } });
        expect(result.valid).toBe(true);
      });

      it('should accept single character name', () => {
        const result1 = validateSkillMetadata({ metadata: { name: 'a', description: validDescription } });
        const result2 = validateSkillMetadata({ metadata: { name: '1', description: validDescription } });
        expect(result1.valid).toBe(true);
        expect(result2.valid).toBe(true);
      });
      ...
      it('should accept name at max length (64 chars)', () => {
        ...
        const result = validateSkillMetadata({ metadata: { name: maxName, description: validDescription } });
        expect(result.valid).toBe(true);
      });
    });
    ...
    describe('invalid names', () => {
      it('should reject empty name', () => {
        const result = validateSkillMetadata({ metadata: { name: '', description: validDescription } });
      ...
      it('should reject name exceeding max length', () => {
        ...
        const result = validateSkillMetadata({ metadata: { name: longName, description: validDescription } });
        expect(result.valid).toBe(false);
      ...
      it('should reject special characters', () => {
        const result1 = validateSkillMetadata({ metadata: { name: 'my_skill', description: validDescription } });
        const result2 = validateSkillMetadata({ metadata: { name: 'my.skill', description: validDescription } });
        const result3 = validateSkillMetadata({ metadata: { name: 'my skill', description: validDescription } });
      ...
      it('should reject consecutive hyphens', () => {
        const result = validateSkillMetadata({ metadata: { name: 'my--skill', description: validDescription } });
        expect(result.valid).toBe(false);
  ...
  describe('description validation', () => {
    ...
    describe('valid descriptions', () => {
      it('should accept normal description', () => {
        ...
        const result = validateSkillMetadata({ metadata: { name: validName, description: desc } });
        expect(result.valid).toBe(true);
      });

      it('should accept single character description', () => {
        const result = validateSkillMetadata({ metadata: { name: validName, description: 'A' } });
        expect(result.valid).toBe(true);
      });

      it('should accept description at max length (1024 chars)', () => {
        const maxDesc = 'a'.repeat(1024);
        const result = validateSkillMetadata({ metadata: { name: validName, description: maxDesc } });
        expect(result.valid).toBe(true);
      });
    ...
    describe('invalid descriptions', () => {
      it('should reject empty description', () => {
        const result = validateSkillMetadata({ metadata: { name: validName, description: '' } });
        expect(result.valid).toBe(false);
        expect(result.errors.some(e => e.includes('cannot be empty'))).toBe(true);
      });
      ...
      it('should reject description exceeding max length', () => {
        ...
        const result = validateSkillMetadata({ metadata: { name: validName, description: longDesc } });
        expect(result.valid).toBe(false);
        expect(result.errors.some(e => e.includes('1024 characters or less'))).toBe(true);
      });

      it('should reject whitespace-only description', () => {
        const result1 = validateSkillMetadata({ metadata: { name: validName, description: '   ' } });
        const result2 = validateSkillMetadata({ metadata: { name: validName, description: '\t\n' } });
        expect(result1.valid).toBe(false);
        expect(result2.valid).toBe(false);
  ...
  describe('compatibility validation', () => {
    ...
    it('should accept empty string', () => {
      const result = validateSkillMetadata({ metadata: { ...validBase, compatibility: '' } });
      expect(result.valid).toBe(true);
    });

    it('should accept undefined (optional)', () => {
      const result = validateSkillMetadata({ metadata: validBase });
      expect(result.valid).toBe(true);
    });
  ...
  describe('license validation', () => {
    ...
    it('should accept common license strings', () => {
      const result1 = validateSkillMetadata({ metadata: { ...validBase, license: 'MIT' } });
      const result2 = validateSkillMetadata({ metadata: { ...validBase, license: 'Apache-2.0' } });
      const result3 = validateSkillMetadata({ metadata: { ...validBase, license: 'BSD-3-Clause' } });
      expect(result1.valid).toBe(true);
      expect(result2.valid).toBe(true);
    ...
    it('should accept empty string', () => {
      const result = validateSkillMetadata({ metadata: { ...validBase, license: '' } });
      expect(result.valid).toBe(true);
    });
  ...
  describe('metadata field validation', () => {
    ...
    it('should accept empty record', () => {
      const result = validateSkillMetadata({ metadata: { ...validBase, metadata: {} } });
      expect(result.valid).toBe(true);
    });
  ...
  describe('full metadata validation', () => {
    ...
    it('should reject missing name', () => {
      const result = validateSkillMetadata({ metadata: { description: 'A skill' } });
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('name'))).toBe(true);
    });

    it('should reject missing description', () => {
      const result = validateSkillMetadata({ metadata: { name: 'my-skill' } });
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('description'))).toBe(true);
    });

    it('should reject invalid name format', () => {
      const result = validateSkillMetadata({ metadata: { name: 'My_Skill', description: 'A skill' } });
      expect(result.valid).toBe(false);
    });

    it('should reject non-object input', () => {
      const result1 = validateSkillMetadata({ metadata: null });
      const result2 = validateSkillMetadata({ metadata: 'string' });
      const result3 = validateSkillMetadata({ metadata: [] });
      expect(result1.valid).toBe(false);
      expect(result2.valid).toBe(false);
      expect(result3.valid).toBe(false);
    });

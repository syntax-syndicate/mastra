/**
 * Repro for https://github.com/mastra-ai/mastra/issues/22618 — regular Agent path.
 *
 ...
 * (agent/durable/workflows/steps/normalize-model-output.ts, shared by the
 * loop's llm-mapping step) used to rewrite it into a V2-style `media` part,
 * stuffing the URL into the Base64-only `data` field and dropping
 * `providerOptions`. The v5→v6 prompt conversion then turned that into
 * `image-data` with the URL still in the Base64-only field.
 */
...
describe('Agent toModelOutput image-url passthrough (issue #22618)', () => {
  it('sends the image-url part untouched to a v3 model on the follow-up step', async () => {
    ...
    const result = await agent.stream('Run the image tool');

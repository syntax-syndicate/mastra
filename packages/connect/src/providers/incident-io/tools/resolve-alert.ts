...
export const resolveAlertInputSchema = z.object({ id: z.string() }).passthrough();
...
export const resolveAlertOutputSchema = ProviderResponseSchema;

export function resolveAlertTool(proxy: PlatformProxy) {
  return createTool({
    id: 'incident_io_resolve_alert',
    description: 'Resolve alert in incident.io.',
    inputSchema: resolveAlertInputSchema,
    outputSchema: resolveAlertOutputSchema,
    execute: async (input, { requestContext }): Promise<z.infer<typeof resolveAlertOutputSchema>> => {

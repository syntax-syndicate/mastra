    ...
    <div className={`rounded-md border ${className}`} style={{ height }}>
      <ScrollArea className="h-full">
        <div className={`group relative p-2 ${onCopy ? 'hover:bg-fill-subtle cursor-pointer' : ''}`}>
          ...
          <pre className="text-meta pointer-events-none font-mono whitespace-pre-wrap">{content}</pre>
          {isDraft && (
            <div className="mt-1.5">
              <span className="text-meta rounded-full bg-yellow-500/20 px-1.5 py-0.5 text-yellow-500">

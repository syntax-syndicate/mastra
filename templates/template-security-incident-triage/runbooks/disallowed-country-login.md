...
Required evidence is the source IP reference, GeoIP result with timestamp and confidence, tenant country policy, and explicit session. Optional evidence includes recent login history, ASN, or network category. Missing history does not imply normal history and does not prevent classification of a verified country signal. GeoIP alone is not proof of compromise.

## Severity Rules

Treat a disallowed country plus verified abnormal session history as high concern. A disallowed country without this aggravating evidence is medium concern; a verified country allowed by the tenant policy is benign. Reduce confidence for known VPN or low-confidence GeoIP results. Missing policy or session evidence requires manual review.

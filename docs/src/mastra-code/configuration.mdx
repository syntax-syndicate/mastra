...
The account manager lists every registered account with its label and marks the active one. From there you can:

- **Add another account** — runs the normal sign-in flow and appends the account.
- **Select an account** — makes it the active account used for new requests.
- **Re-authenticate…** — picks an account and refreshes its credentials by signing in again.
- **Remove…** — deletes an account (after confirmation).

Exactly one account per provider is active at a time; when a request fails over, the provider's other accounts are tried in the order you added them. After a successful sign-in you can name the account — ChatGPT and xAI accounts pre-fill the account's email and GitHub Copilot accounts pre-fill the GitHub login when available. `/logout` still signs out of the provider entirely, removing every registered account.
...
Rotation walks the remaining accounts in the order they were added; each account is tried at most once per request. When every account in the pool has been tried, the error surfaces normally. Other errors — malformed requests (HTTP 400 with no provider usage-limit message) and anything unrecognized — never rotate; they surface immediately.

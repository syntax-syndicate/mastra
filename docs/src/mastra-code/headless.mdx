...
To open the interactive UI with a first message already sent, use `--tui-prompt` instead, or `--tui-initial-prompt` to send it only when there is no earlier conversation for the directory to resume.

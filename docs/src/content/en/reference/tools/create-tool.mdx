...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    description:
      'A human-readable display name for the tool. It is not sent to the model. It travels on the `tool-call` and `tool-call-input-streaming-start` stream chunks and on the stored tool-invocation part, so chat UIs can read it to label the call. MCP clients receive it as the tool `title`.',
    isOptional: true,
  },
  {
    name: 'description',

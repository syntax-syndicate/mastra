...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Vector dimension (must match your embedding model). Required unless `autoEmbed` is set, where the embedding model determines the dimension. Passing both is an error.',
  },
  {
    name: 'autoEmbed',
    type: 'MongoDBAutoEmbedConfig',
    isOptional: true,
    description:
      'Generate the embeddings in MongoDB instead of supplying vectors. See [Automated Embedding](#automated-embedding) for the supported fields.',
  },
  {
    name: 'metric',
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Array of embedding vectors. Required unless the index was created with `autoEmbed`, where MongoDB generates them from `documents` and supplying vectors is an error.',
  },
  {
    name: 'metadata',
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Document text content to store alongside vectors. On an `autoEmbed` index this is the text MongoDB embeds, and it is required.',
  },
]}
/>
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'number[]',
    isOptional: true,
    description: 'Query vector to find similar vectors for. Supply this or `queryText`, not both.',
  },
  {
    name: 'queryText',
    type: 'string',
    isOptional: true,
    description:
      'Text for MongoDB to embed at query time. `autoEmbed` indexes only, and mutually exclusive with `queryVector`. For full-text matching use `textQuery()` instead.',
  },
  {
    name: 'model',
    type: 'string',
    isOptional: true,
    description:
      "Embedding model for this query, overriding the index's. Requires `queryText`, and must be compatible with the index's model.",
  },
  {
    name: 'topK',
    type: 'number',
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'number[]',
    isOptional: true,
    description: 'Query vector for the vector branch. Supply this or `queryText`, not both.',
  },
  {
    name: 'queryText',
    type: 'string',
    isOptional: true,
    description:
      'Text for MongoDB to embed for the vector branch. `autoEmbed` indexes only. Independent of `query`, so each branch can search for something different.',
  },
  {
    name: 'model',
    type: 'string',
    isOptional: true,
    description: "Embedding model for the vector branch, overriding the index's. Requires `queryText`.",
  },
  {
    name: 'query',
    type: 'string',
...
<PropertiesTable
  content={[
  {
    ...
    type: 'string',
    description:
      'Voyage AI model to embed with, for example `voyage-4`. MongoDB rejects a name it does not support and lists the ones it does.',
  },
  {
    name: 'path',
    type: 'string',
    ...
    description:
      'Text field to embed. Defaults to the managed `document` field that `upsert({ documents })` writes. Point it at a field of your own to index an existing collection in place.',
  },
  {
    name: 'similarity',
    type: "'cosine' | 'dotProduct' | 'euclidean'",
    isOptional: true,
    description: "Vector similarity function. Defaults to MongoDB's own default when omitted.",
  },
  {
    name: 'numDimensions',
    ...
    isOptional: true,
    defaultValue: '1024',
    description: 'Length of the generated embeddings.',
  },
  {
    name: 'quantization',
    type: "'float' | 'scalar' | 'binary' | 'binaryNoRescore'",
    isOptional: true,
    defaultValue: 'scalar',
    description: 'Storage format for the generated vectors.',
  },
  {
    name: 'indexingMethod',
    type: "'hnsw' | 'flat'",
    isOptional: true,
    defaultValue: 'hnsw',
    description: 'Index structure for the vector field.',
  },
  {
    name: 'hnswOptions',
    type: '{ maxEdges?: number; numEdgeCandidates?: number }',
    isOptional: true,
    description: "Tuning for the HNSW graph. MongoDB's defaults suit most workloads.",
  },
]}
/>

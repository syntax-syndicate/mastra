...
It can take a tool, agent, or configured classifier as a parameter to automatically create a step from it.
...
Pass a [`Classifier`](/reference/classifier/classifier) with constructor-configured questions to `createStep()`. By default, the classifier evaluates the complete `inputData`. Use `state` to select a JSON value from the step context.

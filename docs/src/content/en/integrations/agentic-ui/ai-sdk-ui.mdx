...
Both part types include delegation metadata for nested supervisor chains:

- `ancestry`: The delegation path in outer-to-inner order. Each entry contains a `toolCallId` and may include the `toolName` and normalized `agentId`.
- `depth`: The number of known agent delegation boundaries. A direct subagent has depth `1`.
- `parentAgentId`: The immediate parent agent ID for a nested agent. This field is omitted for a direct subagent because the root supervisor ID isn't part of the tool envelope.

The part `id` remains the nested agent run ID, so progressive snapshots replace the same UI state as the run continues.
            ...
            <div key={message.id}>
              {message.parts.map((part, index) => {
                ...
                if (part.type === 'data-tool-agent') {
                  const { id, data, depth, parentAgentId } = part as AgentDataPart
                  return (
                    <div key={index} className="nested-agent">
                      <strong>Nested Agent: {id}</strong>
                      <p>Delegation depth: {depth}</p>
                      {parentAgentId && <p>Parent agent: {parentAgentId}</p>}
...
- `AgentDataPart` and `AgentStepDataPart` include the same `ancestry`, `depth`, and optional `parentAgentId` fields

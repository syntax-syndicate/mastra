...
description: 'Create, list, aggregate, and export human feedback records in Mastra observability, with ratings and comments linked to specific traces and spans.'
packages:
  - '@mastra/core'
...
Filters include target fields such as `traceId` and `spanId`, feedback fields such as `feedbackType`, `feedbackSource`, and `feedbackUserId`, and shared context fields such as `entityName`, `environment`, `experimentId`, and `tags`. See [`FeedbackFilter`](/reference/observability/feedback#feedbackfilter) for every filter field. Over HTTP, the same fields are passed as query parameters, for example `GET /api/observability/feedback?traceId=trace-123&environment=production`. See [list query parameters](/reference/observability/feedback#list-query-parameters).
...
The local runtime exposes the list route at `/api/observability/feedback` and analytics under its related paths. See the [HTTP routes table](/reference/observability/feedback#http-routes) and [list query parameters](/reference/observability/feedback#list-query-parameters). Mastra Platform provides a separate, unversioned hosted query API. See the [observability feedback query API](/docs/mastra-platform/api#observability-feedback-query-api) for regional endpoints, authentication, and project scoping.

...
The function can be asynchronous. For decisions that depend on the tool name and arguments, use a [classifier](/reference/classifier/classifier) to estimate whether the call needs human review:

```typescript title="src/mastra/agents/run-with-approval.ts"

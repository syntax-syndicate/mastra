...
![Aster refunds dataset in Studio, showing five rows with Input and Ground Truth columns. Each ground truth contains must and mustNot criteria.](/img/guides/build-an-eval-loop/aster-refunds-dataset.png)
...
![Run Experiment dialog configured as Baseline: Aster refunds version 1 with five items, Aster Space Reservations as the target agent, and one scorer selected.](/img/guides/build-an-eval-loop/run-experiment-dialog.png)
...
![Baseline results: all five cases score 1.000 for refund correctness. The pipeline shows the dataset feeding the agent, then the scorer.](/img/guides/build-an-eval-loop/experiment-results.png)
...
![Run Experiment dialog named 5.4-mini, with the same Aster refunds version 1 dataset, Aster Space Reservations agent, and one scorer selected.](/img/guides/build-an-eval-loop/run-experiment-model-change.png)
...
![Experiments list showing Baseline and 5.4-mini, each marked Run completed with five processed items.](/img/guides/build-an-eval-loop/model-experiments-list.png)

Select **Compare**, choose both experiments, then select **Compare Experiments**.

![Comparison selection mode with Baseline and 5.4-mini checked, a 2/2 selected indicator, and the Compare Experiments button ready.](/img/guides/build-an-eval-loop/select-experiments-to-compare.png)
...
![Baseline and 5.4-mini answers side by side for a customer who accepted a mission assignment. Both deny the refund and score 1.00. The baseline answer is shorter. Timings for this case are 5.23 and 6.18 seconds, respectively.](/img/guides/build-an-eval-loop/model-experiment-comparison.png)
...
![Trace of an eligible customer requesting a refund. The agent asks for a reservation ID, full name, and payment method so it can proceed, despite having no refund tools. The trace menu highlights Add full trace to dataset.](/img/guides/build-an-eval-loop/production-trace-add-to-dataset.png)
...
![Save as Dataset Item form targeting Aster refunds. The input is prefilled from the trace. Ground truth requires confirming eligibility and directing the customer to support, and prohibits offering to process the refund or collecting booking details for that purpose.](/img/guides/build-an-eval-loop/production-trace-ground-truth.png)
...
![Failed result for a customer who already accepted a mission assignment. The agent promises a refund because payment was within 14 days. The judge assigns 0 and explains that the answer ignores the accepted-assignment restriction.](/img/guides/build-an-eval-loop/failed-case-judge-reason.png)
